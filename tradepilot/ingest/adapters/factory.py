...
def build_reference_adapter(settings: Settings):
    if settings.reference_adapter_kind == "api":
        return ReferenceApiAdapter(endpoint=settings.reference_api_endpoint)
    return ReferenceApiAdapter(endpoint=settings.reference_api_endpoint)
